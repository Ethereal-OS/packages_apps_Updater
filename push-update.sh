...
updates_dir=/data/etherealos_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have Ethereal-VERSION-DATE-TYPE-* as format"
...
# Assume Ethereal-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.ethereal.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.ethereal.updater/databases/updates.db" \
